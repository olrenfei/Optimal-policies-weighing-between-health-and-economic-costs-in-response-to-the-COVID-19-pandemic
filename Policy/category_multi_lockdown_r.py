# -*- coding: utf-8 -*-
"""
Created on Wed Dec 22 18:38:08 2021
...
def Result(category,region_portion,SIARa,SIARb,SIARc,beta,gamma,alpha,r,dt,days,step,Mij3_1):#根据给定参数模拟传染病传播过程,得到SIAR随时间变化情况与每日新增确诊病例数
    B_policy=[0]
    A_policy=[0]
    C_policy=[0]
    ...
    for k in range(step-1):
        ...
        if k==(20*24):
            ...
            SIARa_lockdown,SIARa_move=split_stop_move(SIARa,PL)
            SIARb_lockdown,SIARb_move=split_stop_move(SIARb,PL)
            SIARc_lockdown,SIARc_move=split_stop_move(SIARc,PL)
            #自由活动个体的感染与移动
            ...
            D=mobility_without_zero[indexk]
            SIARa = Move5(SIARanew_move,D)
            SIARb = Move5(SIARbnew_move,D)
            SIARc = Move5(SIARcnew_move,D)
            #被隔离个体的状态变化
            ...
            blockdown=Sb_stop+Ib_stop+Ab_stop+Rb_stop
            B_policy.append(dt*blockdown)
            alockdown=Sa_stop+Ia_stop+Aa_stop+Ra_stop
            A_policy.append(dt*alockdown)
            clockdown=Sc_stop+Ic_stop+Ac_stop+Rc_stop
            C_policy.append(dt*clockdown)
            resulta.append((sum(SIARa['S'])+Sa_stop, sum(SIARa['I'])+Ia_stop, sum(SIARa['A'])+Aa_stop, sum(SIARa['R'])+Ra_stop))
            resultb.append((sum(SIARb['S'])+Sb_stop, sum(SIARb['I'])+Ib_stop, sum(SIARb['A'])+Ab_stop, sum(SIARb['R'])+Rb_stop))
            resultc.append((sum(SIARc['S'])+Sc_stop, sum(SIARc['I'])+Ic_stop, sum(SIARc['A'])+Ac_stop, sum(SIARc['R'])+Rc_stop))
            
            ###模拟正常状态下被隔离个体的移动，为了计算正常状态下每个子区域个体数
        ...
        if k>(20*24):
            ...
            ##假设每个人去某一类型子区域的概率相同
            PL=region_portion[indexk][:,category]
            #PL[PL<1]=0##假设未被隔离的所有人均不再去被隔离子区域(pl<1,移动不变；pl=1,在家)
            ...
            ###第三类，自由活动的人，感染与移动
            SIARanew_move,NIa = SIAR_P(a_Norm,b_Norm,c_Norm,SIARa_move,0,0.34,SIARa_move,SIARb_move,SIARc_move,Mij3,beta,gamma,alpha,r,dt)
            SIARbnew_move,NIb = SIAR_P(a_Norm,b_Norm,c_Norm,SIARb_move,1,1,SIARa_move,SIARb_move,SIARc_move,Mij3,beta,gamma,alpha,r,dt)
            SIARcnew_move,NIc = SIAR_P(a_Norm,b_Norm,c_Norm,SIARc_move,2,1.47,SIARa_move,SIARb_move,SIARc_move,Mij3,beta,gamma,alpha,r,dt)            
            NI.append(sum(NIa)+sum(NIb)+sum(NIc))
            ...
            SIARb = Move5(SIARbnew,D)
            SIARc = Move5(SIARcnew,D)
            B_policy.append(dt*(sum(SIARb_stop['NUM'])+blockdown))
            A_policy.append(dt*(sum(SIARa_stop['NUM'])+alockdown))
            C_policy.append(dt*(sum(SIARc_stop['NUM'])+clockdown))
...
def cal_para(category,region_portion,r, SIARa0, SIARb0, SIARc0, g, alpha, beta, gamma, Xunhuan, i):
    ...
    for xunhuan in range(Xunhuan):
        resulta,resultb,resultc,NewConfirmed_hat,B_policy,A_policy,C_policy= Result(category,region_portion,SIARa0,SIARb0,SIARc0,beta,gamma,alpha,r,dt,days,step,Mij3_1)
        #resulta,resultb,resultc,NewConfirmed_hat,B_policy= Result_no_lockdown(category,region_portion,A_Norm,B_Norm,C_Norm,SIARa0,SIARb0,SIARc0,beta,gamma,alpha,r,dt,days,step,Mij3_1)
