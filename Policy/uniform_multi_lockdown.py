# -*- coding: utf-8 -*-
"""
Created on Tue Dec 14 19:38:56 2021
...
def Result(pl,SIARa,SIARb,SIARc,beta,gamma,alpha,r,dt,days,step,Mij3_1):#根据给定参数模拟传染病传播过程,得到SIAR随时间变化情况与每日新增确诊病例数
    ...
    for k in range(step-1):
        ...
        if (x<20):
            ...
            SIARanew_move,NIa = SIAR(SIARa_move,0,0.34,SIARa_move,SIARb_move,SIARc_move,Mij3,beta,gamma,alpha,r,dt)
            SIARbnew_move,NIb = SIAR(SIARb_move,1,1,SIARa_move,SIARb_move,SIARc_move,Mij3,beta,gamma,alpha,r,dt)
            SIARcnew_move,NIc = SIAR(SIARc_move,2,1.47,SIARa_move,SIARb_move,SIARc_move,Mij3,beta,gamma,alpha,r,dt)
            NI.append(sum(NIa)+sum(NIb)+sum(NIc))
            indexk=keys[(k-71)%168]
            D=mobility_without_zero[indexk]
    ...
    return resulta,resultb,resultc,NewConfirmed_hat,B_policy
...
def cal_para(pl,r, SIARa0, SIARb0, SIARc0, g, alpha, beta, gamma, Xunhuan, i):
    ...
    for xunhuan in range(Xunhuan):
        resulta,resultb,resultc,NewConfirmed_hat,B_policy= Result(pl,SIARa0,SIARb0,SIARc0,beta,gamma,alpha,r,dt,days,step,Mij3_1)
    ...
    subname='bi_multi_uniform'+str(int(pl*100))
