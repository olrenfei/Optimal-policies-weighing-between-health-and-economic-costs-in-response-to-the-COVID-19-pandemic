...
para1 = pd.read_csv('1125Luo_Para_104.csv')#要修改为待估参数文件名
...
def main(para1,Rate,dt,days,step,Mij3_1,N,numI,area,age,susc,pool):
    ...
    for m in range(len(Rate)):
        ...
        for i in range(len(para)):
            ...
            #print(results.get())
            # cal_para(r, SIARa0, SIARb0, SIARc0, g, alpha, beta, gamma, Xunhuan, i)
...
def cal_para(r, SIARa0, SIARb0, SIARc0, g, alpha, beta, gamma, Xunhuan, i):
    ...
    Para = pd.DataFrame({'Beta':[beta],'Gamma':[gamma],'Alpha':[alpha],'Rate':[r],'RMSE':[Ave_rmse]})
    
    Para.to_csv("".join([str(g), 'Para_hat_',subname,'.tsv']),encoding='utf_8_sig')
    Ave_resulta=pd.DataFrame(Ave_resulta)
    Ave_resulta.to_csv("".join([str(g), 'SIAR_',subname, '.tsv']),encoding='utf_8_sig')
    Ave_resultb=pd.DataFrame(Ave_resultb)
    Ave_resultb.to_csv("".join([str(g), 'SIARa_',subname, '.tsv']),encoding='utf_8_sig')
    Ave_resultc=pd.DataFrame(Ave_resultc)
    Ave_resultc.to_csv("".join([str(g), 'SIARb_',subname, '.tsv']),encoding='utf_8_sig')
    Ave_result=pd.DataFrame(Ave_result)
    Ave_result.to_csv("".join([str(g), 'SIARc_',subname, '.tsv']),encoding='utf_8_sig')
...
#    pool.join()
    start = time.time()
    pool = mp.Pool(mp.cpu_count())
    main(para1,Rate,dt,days,step,Mij3_1,N,numI,area,age,susc,pool)
    pool.close()
    pool.join()
    end = time.time()
    print(end-start)
